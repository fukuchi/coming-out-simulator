function Start(){
	...
	N("「半分の真実」についての、半分本当の物語。");
	N("やあプレイヤー君、このゲームへようこそ。ざっと20分くらいかな、お付き合いいただくよ。");
	N("さて、まずは何をしたい?");
...
function Play_3(){
	...
	if(!$.asked_credits){
		N("君が「作者紹介」をとばしたから、あらためて伝えよう。僕の（まだ法的には有効じゃない）名前はニッキー・ケース。これで覚えてもらえたかな。");
		p("チッ。");
	}
...
function Credits(message){
	...
	N("そう。この『Coming Out Simulator』のシナリオライター・プログラマー・アーティストを僕一人でやっている。");

	if($.asked_about){
		p("ぜんぶ自分で??");
		p("前にも言ったけど、君さぁ...");
		p("かなりのナルシストだよ。");
		N("あ、本当にぜんぶを自分で、って訳じゃないんだ。");
		N("効果音はいろんな無料素材を使っている。");
	}else{
		N("もっとも、効果音はいろんな無料素材を使っているけどね。");
	}

	N("あと、たしかに僕はこのゲームに出ずっぱりだけど...");
	N("...このゲームのストーリー自体には、沢山の人が関わっているんだ。");
...
function About(message){
	...
	N("そうだよ。");
	N("さてさて! このゲームは著作権を放棄している。パブリックドメインなんだ。");
